...
void summarizeText(const std::string& text) {
    ...
    while (std::getline(iss, line, '.')) { // Split by '.' for sentences
        if (!line.empty()) {
            sentences.push_back(line + '.'); // Add the period back to each sentence
        }
    }
...
int main() {
    ...
    if (choice == 1) {
        ...
        // 메모리 해제
        delete[] utf_text.c_str(); // This is not necessary since utf_text is a std::string
        api->End();
        pixDestroy(&image);
